...
public sealed class CanonicalAudioAnalysisResult {
 public string CodecFamily="Unknown",PayloadFormat="Unknown",FirstError="",DolbyCodec="",DolbyLayout=""; public long Samples,PayloadBytes,Minimum=Int64.MaxValue,Maximum,Rejected,MpegAudioFrames,MpegAudioBytes,MpegAudioPcmSamples,DolbyFrames,DolbyBytes,DolbyRejected,DolbyPcmSamples; public double DurationSeconds,Bitrate,Average; public int MpegVersion,Layer,SampleRate,Channels,MinimumFrameBitrate=Int32.MaxValue,MaximumFrameBitrate,DolbySampleRate,DolbyChannels;
...
public static class CanonicalAudioAnalyzer {
 ...
 public static CanonicalAudioAnalysisResult Analyze(string path,LsdTrackModel track,double duration){var r=new CanonicalAudioAnalysisResult();if(track==null)return r;r.CodecFamily=track.CodecFamily;r.PayloadFormat=track.PayloadFormat;r.DurationSeconds=duration;using(var fs=new System.IO.FileStream(path,System.IO.FileMode.Open,System.IO.FileAccess.Read,System.IO.FileShare.ReadWrite)){foreach(var s in track.Samples){try{if(s.FileOffset<0||s.Length<=0||s.FileOffset+s.Length>fs.Length)throw new System.IO.InvalidDataException("Canonical audio sample is outside file bounds");r.Samples++;r.PayloadBytes+=s.Length;if(s.Length<r.Minimum)r.Minimum=s.Length;if(s.Length>r.Maximum)r.Maximum=s.Length;if(track.CodecFamily=="MP3")ParseMp3(fs,s,r);else if(track.CodecFamily=="AC3"||track.CodecFamily=="EAC3")ParseDolby(fs,s,r);}catch(Exception ex){r.Rejected++;if(r.FirstError.Length==0)r.FirstError=ex.GetType().Name+": "+ex.Message;}}}if(r.Samples>0){r.Average=(double)r.PayloadBytes/r.Samples;if(duration>0)r.Bitrate=r.PayloadBytes*8.0/duration;if(r.MpegAudioPcmSamples>0&&r.SampleRate>0)r.Bitrate=r.MpegAudioBytes*8.0/((double)r.MpegAudioPcmSamples/r.SampleRate);if(r.DolbyPcmSamples>0&&r.DolbySampleRate>0)r.Bitrate=r.DolbyBytes*8.0/((double)r.DolbyPcmSamples/r.DolbySampleRate);}if(r.Minimum==Int64.MaxValue)r.Minimum=0;if(r.MinimumFrameBitrate==Int32.MaxValue)r.MinimumFrameBitrate=0;return r;}
}
...
// RIFF AVI 1.0 reader with idx1 canonical sample preparation.
...
public sealed class AviProbeResult {
 public NativeMediaInfo Info; public long MoviData; public readonly System.Collections.Generic.List<AviStreamData> Streams=new System.Collections.Generic.List<AviStreamData>();
...
public static class NativeAviProbe {
 ...
 public static AviProbeResult Inspect(string path){var r=new AviProbeResult();var n=new NativeMediaInfo();var fi=new System.IO.FileInfo(path);n.Length=fi.Length;n.ModifiedUtc=fi.LastWriteTimeUtc;n.Container="AVI";n.Detail="RIFF AVI 1.0";n.DocType="avi";r.Info=n;using(var s=new System.IO.FileStream(path,System.IO.FileMode.Open,System.IO.FileAccess.Read,System.IO.FileShare.ReadWrite)){if(Four(s)!="RIFF"){throw new System.IO.InvalidDataException("AVI RIFF header was not found");}U32(s);if(Four(s)!="AVI ")throw new System.IO.InvalidDataException("RIFF form is not AVI");long idx=-1,idxSize=0,p=12;while(p+8<=s.Length){s.Position=p;string id=Four(s);uint z=U32(s);long data=p+8,next=data+z+(z&1);if(next>s.Length||next<data)throw new System.IO.InvalidDataException("Invalid AVI chunk "+id);if(id=="LIST"&&z>=4){string type=AtFour(s,data);long body=data+4;if(type=="hdrl"){long q=body;while(q+8<=next){s.Position=q;string sid=Four(s);uint sz=U32(s);long sd=q+8,sn=sd+sz+(sz&1);if(sn>next||sn<sd)break;if(sid=="LIST"&&sz>=4&&AtFour(s,sd)=="strl")ParseStrl(s,sd+4,sn,r);q=sn;}}else if(type=="movi")r.MoviData=body;else if(type=="INFO")ParseInfo(s,body,next,n);}else if(id=="idx1"){idx=data;idxSize=z;}p=next;}if(r.MoviData<=0)throw new System.IO.InvalidDataException("AVI movi list was not found");if(idx<0)throw new System.IO.InvalidDataException("AVI 1.0 idx1 index was not found; OpenDML is not enabled in this stage");s.Position=idx;long entries=idxSize/16;long[] counters=new long[Math.Max(1,r.Streams.Count)];for(long i=0;i<entries;i++){string cid=Four(s);uint flags=U32(s),off=U32(s),len=U32(s);int si=-1;if(cid.Length>=2)Int32.TryParse(cid.Substring(0,2),out si);if(si<0||si>=r.Streams.Count)continue;var st=r.Streams[si];if(len==0)continue;long payload=ResolveChunk(s,r.MoviData,s.Length,cid,off,len);s.Position=idx+(i+1)*16;if(payload<0)throw new System.IO.InvalidDataException("AVI idx1 entry cannot be resolved: "+cid+" @ "+off);long k=counters[si]++;if(st.Native.Type==1){long ts=k;r.VideoSamples.Add(new LsdSampleModel{TrackId=st.Native.Number,FileOffset=payload,Length=len,DecodeTimestamp=ts,PresentationTimestamp=ts,IsKeyFrame=(flags&0x10)!=0,PayloadFormat=st.Native.CodecId=="V_MPEG4/ISO/AVC"?"AnnexBNal":"Mpeg4Visual"});}else if(st.Native.Type==2){st.Native.AudioPayloadBytes+=len;r.AudioSamples.Add(new LsdSampleModel{TrackId=st.Native.Number,FileOffset=payload,Length=len,DecodeTimestamp=k,PresentationTimestamp=k,IsKeyFrame=true,PayloadFormat=st.Native.CodecId=="A_MPEG/L3"?"MpegAudio":"RawCodecFrame"});}}PopulateAviAvc(s,r);foreach(var audioStream in r.Streams){if(audioStream.Native!=null&&audioStream.Native.Type==2&&audioStream.Rate>0){double audioSeconds=(double)audioStream.Length*audioStream.Scale/audioStream.Rate;if(audioSeconds>0)audioStream.Native.AudioBitrate=audioStream.Native.AudioPayloadBytes*8.0/audioSeconds;}}return r;}}
 public static LsdMediaModel Adapt(string path,AviProbeResult p){var m=LsdMatroskaAdapter.Adapt(path,p.Info);m.ContainerType="AVI";m.ContainerDetail="RIFF AVI 1.0 + idx1";m.AdapterName="AviToLsdAdapter/2.0";m.TimecodeScale=1000000;foreach(var t in m.Tracks){if(t.TrackType=="video"){t.PayloadFormat=t.CodecFamily=="MPEG4ASP"?"Mpeg4Visual":t.CodecFamily=="AVC"?"AnnexBNal":t.PayloadFormat;foreach(var x in p.VideoSamples)if(x.TrackId==t.TrackId){var st=p.Streams[(int)t.TrackId];long ns=st.Rate>0?(long)Math.Round(1000000000.0*st.Scale/st.Rate):1000000;m.TimecodeScale=ns;x.PresentationTimestamp=x.DecodeTimestamp;t.Samples.Add(x);}}else if(t.TrackType=="audio"){foreach(var x in p.AudioSamples)if(x.TrackId==t.TrackId)t.Samples.Add(x);}}return m;}
}
public sealed class NativeAviPreparationJob : IDisposable {
 ...
 public void Start(string path){thread=new System.Threading.Thread(()=>Run(path));thread.IsBackground=true;thread.Name="LSD AVI idx1 preparation";thread.Start();}
...
function Render {
    ...
    if($script:ReadyMedia -and $script:NativeInfo.Container -eq 'AVI') {
        ...
        $lines+=@('','[ Native AVI Sample Index ]','',('Video samples:         {0:N0}' -f $(if($aviVideo){$aviVideo.Samples.Count}else{0})),'Index mode:            AVI 1.0 idx1','OpenDML AVI:           Not enabled in this stage','Video mapping:         XVID/DIVX/DX50/FMP4/MP4V -> MPEG-4 Part 2 ASP; H264/X264/AVC1/DAVC -> AVC Annex B')
    }
...
function Build-NativeMetadata {
    ...
    foreach($t in $script:NativeInfo.Tracks){
        ...
        $codecLong=switch($t.CodecId){'V_MPEG4/ISO/AVC'{'H.264 / AVC'};'V_MPEGH/ISO/HEVC'{'H.265 / HEVC'};'V_AV1'{'AV1'};'V_MPEG4/ISO/ASP'{'MPEG-4 Part 2 / XviD'};'A_MPEG/L3'{'MP3'};'A_AC3'{'Dolby Digital / AC-3'};'A_EAC3'{'Dolby Digital Plus / E-AC-3'};'A_PCM'{'PCM'};'A_AAC'{'AAC'};default{$t.CodecId}}
        ...
        elseif($script:ScanJob) {
            ...
            if($script:ScanJob.Done) {
                ...
                if(-not $script:Cancelled -and $j.ExitCode -eq 0) {
                    ...
                    $script:PacketEngine=if($script:NativeInfo.Container -eq 'MP4 / MOV'){'Canonical LSD media pipeline (Native MP4 sample tables)'}elseif($script:NativeInfo.Container -eq 'AVI'){'Canonical LSD media pipeline (Native AVI idx1)'}else{'Canonical LSD media pipeline'}
