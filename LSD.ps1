...
public static class NativeMp4Probe {
 ...
 static void Stsd(System.IO.Stream s,Mp4Box b,Mp4TrackData t){s.Position=b.Data+4;uint count=U32(s);if(count==0)return;long entryStart=s.Position;uint size=U32(s);string four=Four(s);long payload=s.Position,end=entryStart+size;if(size<8||end>b.End)throw new System.IO.InvalidDataException("Invalid stsd entry");t.CodecFourcc=four;var n=new NativeTrack();n.Number=t.TrackId;n.Language="und";if(t.Handler=="vide"){
   n.Type=1;s.Position=payload+24;n.Width=U16(s);n.Height=U16(s);long child=payload+78;if(four=="avc1"||four=="avc3"){n.CodecId="V_MPEG4/ISO/AVC";n.CodecPrivate=FindConfig(s,child,end,"avcC");}else if(four=="hvc1"||four=="hev1"){n.CodecId="V_MPEGH/ISO/HEVC";n.CodecPrivate=FindConfig(s,child,end,"hvcC");}else n.CodecId="V_"+four.ToUpperInvariant();
...
function Render-QpHistogram {
    ...
    if($null -eq $script:QpCounts -or $script:QpCounts.Count -eq 0){
        $qpStatsText.Lines=@('Frames : N/A','Avg    : N/A','Median : N/A','Mode   : N/A','Range  : N/A','Peak   : N/A')
        $qpText.Lines=@('DRF distribution is generated by the native frame parser.','','Native analysis starts automatically after metadata detection.')
        return
    }
    ...
    $displayMax=[Math]::Min(69,[int](($items | Measure-Object DRF -Maximum).Maximum)+3)
    for($drf=$displayMin;$drf -le $displayMax;$drf++){
        ...
        $bar=('█' * $barLength)
        $lines.Add(('{0,2} | {1,8:N0} | {2,-10} | {3,5:N1}%' -f $drf,$count,$bar,$pct))
    }
    ...
    $qpStatsText.Lines=@(('Frames : {0:N0}' -f $totalFrames),('Avg    : {0:N1}' -f $average),('Median : {0}' -f $medianText),('Mode   : {0}' -f $mode.DRF),('Range  : {0}-{1}' -f $minDrf,$maxDrf),('Peak   : {0} ({1:N1}%)' -f $mode.DRF,$peakPct))
...
function Render {
    ...
    if($script:ReadyMedia -and $script:NativeInfo.Container -eq 'MP4 / MOV') {
        ...
        $lines+=@('','[ Native MP4 Sample Index ]','',('Video samples:         {0:N0}' -f $mp4SampleCount),('Timecode scale:        {0:N0} ns/tick' -f $script:ReadyMedia.TimecodeScale),'Sample table mode:     stsc + stsz + stco/co64 + stts/ctts + stss','Fragmented MP4:        Not supported in version 2.0')
    }
        ...
        elseif($script:ScanJob) {
            ...
            if($script:ScanJob.Done) {
                ...
                if(-not $script:Cancelled -and $j.ExitCode -eq 0) {
                    ...
                    Log ('LSD 2.0 pipeline: '+$script:NativeInfo.Container+' reader -> canonical sample index -> shared AVC/HEVC analyzer.')
                }
