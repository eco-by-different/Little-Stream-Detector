...
function Render-QpHistogram {
    ...
    if($null -eq $script:QpCounts -or $script:QpCounts.Count -eq 0){
        $qpText.Lines=@(
            'DRF distribution is generated by the native frame parser.',
            '',
    ...
    $items=@(
        $script:QpCounts.GetEnumerator() |
            ForEach-Object {
                [pscustomobject]@{DRF=[int]$_.Key;Count=[long]$_.Value}
            } |
    ...
    for($drf=0;$drf -le 69;$drf++){
        ...
        $bar=('█' * $barLength)
        $lines.Add(('{0,2} | {1,10:N0} | {2,-20} | {3,5:N1}%' -f $drf,$count,$bar,$pct))
    }
    ...
    [long]$running=0
    $leftMedian=$null;$rightMedian=$null
    foreach($item in $byDrf){
        $running+=$item.Count
        if($null -eq $leftMedian -and $running -ge $leftTarget){$leftMedian=$item.DRF}
        if($null -eq $rightMedian -and $running -ge $rightTarget){$rightMedian=$item.DRF;break}
    }
    ...
    $qpStatsText.Lines=@(
        ('Frames : {0:N0}' -f $totalFrames),
        ('Avg    : {0:N1}' -f $average),
        ('Median : {0}' -f $medianText),
        ('Mode   : {0}' -f $mode.DRF),
        ('Range  : {0}-{1}' -f $minDrf,$maxDrf),
        ('Peak   : {0} ({1:N1}%)' -f $mode.DRF,$peakPct)
    )
    # Keep the complete 0..69 scale, but open the view at the dominant DRF value.
    $peakLine=[Math]::Max(0,[Math]::Min(69,[int]$mode.DRF))
    $peakChar=$qpText.GetFirstCharIndexFromLine($peakLine)
    if($peakChar -ge 0){
        $qpText.SelectionStart=$peakChar
...
$chartBox=New-Object Windows.Forms.GroupBox;$chartBox.Text='Bitrate profile';$chartBox.SetBounds(20,198,914,142);$chartBox.Anchor='Top,Left,Right';$chart=New-Object Windows.Forms.Panel;$chart.Dock='Fill';$chart.BackColor=$RoyalPanel;$chartBox.Controls.Add($chart);$chart.Add_Resize({$chart.Invalidate($true)});$qpBox=New-Object Windows.Forms.GroupBox;$qpBox.Text='DRF distribution (frame-level SliceQPY)';$qpBox.SetBounds(20,348,914,154);$qpBox.Anchor='Top,Left,Right';$qpSplit=New-Object Windows.Forms.SplitContainer;$qpSplit.Dock='Fill';$qpSplit.Orientation='Vertical';$qpSplit.FixedPanel='Panel2';$qpText=New-Object Windows.Forms.TextBox;$qpText.Multiline=$true;$qpText.ReadOnly=$true;$qpText.WordWrap=$false;$qpText.ScrollBars='Both';$qpText.Dock='Fill';$qpText.Font=New-Object Drawing.Font('Consolas',9);$qpText.BackColor=$RoyalPanel;$qpText.ForeColor=$RoyalText;$qpStatsText=New-Object Windows.Forms.TextBox;$qpStatsText.Multiline=$true;$qpStatsText.ReadOnly=$true;$qpStatsText.WordWrap=$false;$qpStatsText.ScrollBars='None';$qpStatsText.Dock='Fill';$qpStatsText.Font=New-Object Drawing.Font('Consolas',9);$qpStatsText.BackColor=$RoyalPanel;$qpStatsText.ForeColor=$RoyalText;$qpSplit.Panel1.Controls.Add($qpText);$qpSplit.Panel2.Controls.Add($qpStatsText);$qpBox.Controls.Add($qpSplit);$qpSplit.Add_SizeChanged({if($qpSplit.Width -gt 650){$target=[Math]::Max(420,$qpSplit.Width-220);if($target -lt $qpSplit.Width){$qpSplit.SplitterDistance=$target}}});$tabs=New-Object Windows.Forms.TabControl;$tabs.SetBounds(20,510,914,260);$tabs.Anchor='Top,Bottom,Left,Right'
