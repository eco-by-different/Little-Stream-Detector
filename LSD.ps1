...
readonly double duration; readonly int nalLengthSize; readonly H264SliceHeaderConfig sliceConfig; readonly bool avcMode,hevcMode,av1Mode; readonly LsdTrackModel readyTrack; readonly long readyTimecodeScale; readonly NativeTrack sourceTrack; readonly int hevcExtraSliceHeaderBits; readonly HevcHvccProbeResult hevcProbeContext; long hevcPrefixParsed,hevcPrefixRejected; string hevcPrefixFirstError=""; readonly long[] bins=new long[240]; readonly long[] qpHistogram=new long[64]; readonly StringBuilder hevcTypeSequence=new StringBuilder();
...
function Start-Scan {
    ...
    try {
        ...
        if($script:NativeInfo.Container -eq 'MP4 / MOV'){$script:ScanPhase='canonical';$script:ScanJob.StartCanonical($script:File)}else{$script:ScanPhase='container-index';$script:ScanJob.Start($script:File,[long]$nativeVideo.Number)}
        Busy $true
        $bar.Value=0
