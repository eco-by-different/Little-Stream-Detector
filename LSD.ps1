...
public static class NativeMediaProbe {
 ...
 static void ParsePps(byte[] nal,NativeTrack t){var b=new Bits(Rbsp(nal,1,nal.Length-1));b.UE();b.UE();t.Cabac=b.Bit()!=0;t.BottomFieldPicOrderInFramePresent=b.Bit()!=0;uint groups=b.UE();if(groups>0)return;t.NumRefL0=(int)b.UE();t.NumRefL1=(int)b.UE();t.WeightedPred=b.Bit()!=0;t.WeightedBipred=(int)b.U(2);t.PicInitQpMinus26=b.SE();b.SE();b.SE();t.DeblockingFilterControlPresent=b.Bit()!=0;b.Bit();t.RedundantPicCntPresent=b.Bit()!=0;if(b.Remaining>8){t.Transform8x8=b.Bit()!=0;if(b.Bit()!=0){int scalingCount=t.Chroma=="YUV 4:4:4"?12:8;for(int i=0;i<scalingCount;i++)if(b.Bit()!=0)Scaling(b,i<6?16:64);}b.SE();}}
...
public sealed class HevcSpsProbeResult {
 ...
 public bool SampleAdaptiveOffsetEnabled,TemporalMvpEnabled,LongTermRefsPresent;
