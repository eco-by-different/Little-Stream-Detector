...
using System; using System.Text; using System.Runtime.InteropServices; using System.Globalization;
...
public sealed class NativeMediaInfo {
 ...
 public string DocType=""; public long TimecodeScale=1000000; public double DurationSeconds; public string MuxingApp=""; public string WritingApp=""; public readonly System.Collections.Generic.List<NativeTrack> Tracks=new System.Collections.Generic.List<NativeTrack>();
 public void AssignSoftware(string value){value=(value??"").Trim();if(value.Length==0)return;string lower=value.ToLowerInvariant();if(lower.StartsWith("lavf")||lower.Contains("libavformat")){if(MuxingApp.Length==0)MuxingApp=value;}else if(WritingApp.Length==0)WritingApp=value;}
}
public sealed class NativeTrack {
 ...
 public bool Cabac,WeightedPred,Transform8x8,Sbr,Ps,HevcTier,HevcDependentSliceSegments,HevcOutputFlagPresent,HevcCabacInitPresent,HevcCuQpDeltaEnabled,HevcWeightedPred,HevcWeightedBipred,HevcTilesEnabled,HevcWppEnabled,HevcDeblockingControlPresent,HevcDeblockingOverrideEnabled,HevcLoopFilterAcrossSlices,HevcListsModificationPresent; public int HevcProfile,HevcLevel,HevcTemporalLayers,HevcExtraSliceHeaderBits,HevcNumRefL0,HevcNumRefL1,HevcPicInitQpMinus26,HevcDiffCuQpDeltaDepth; public int WeightedBipred; public string ColorPrimaries="",ColorTransfer="",ColorMatrix="",ColorRange=""; public int CoreSampleRate,OutputSampleRate,CoreAudioChannels,AudioChannels; public string AudioProfile="",PcmFormat=""; public long AudioPayloadBytes; public double AudioBitrate; public int PcmBitDepth,PcmBlockAlign; public bool PcmFloat,PcmBigEndian;
...
public sealed class LsdTrackModel {
 ...
 public byte[] CodecPrivate; public int NalLengthSize,PcmBitDepth,PcmBlockAlign; public bool PcmFloat,PcmBigEndian; public long Width,Height,Channels; public double SamplingRate;
...
public static class LsdMatroskaAdapter {
 static string Family(string id){if(id=="V_MPEG4/ISO/AVC")return "AVC";if(id=="V_MPEGH/ISO/HEVC")return "HEVC";if(id=="V_AV1")return "AV1";if(id=="V_MPEG4/ISO/ASP")return "MPEG4ASP";if(id=="V_VP9")return "VP9";if(id=="A_AAC")return "AAC";if(id=="A_EAC3"||id=="A_E-AC-3")return "EAC3";if(id!=null&&id.StartsWith("A_AC3"))return "AC3";if(id!=null&&id.StartsWith("A_MPEG/L3"))return "MP3";if(id!=null&&id.StartsWith("A_PCM"))return "PCM";return "Unknown";}
 static string Payload(string f){if(f=="AVC"||f=="HEVC")return "LengthPrefixedNal";if(f=="AV1")return "Av1Obu";if(f=="MPEG4ASP")return "Mpeg4Visual";if(f=="VP9")return "Vp9Frame";return "RawCodecFrame";}
 public static LsdMediaModel Adapt(string path,NativeMediaInfo n){if(n==null)throw new ArgumentNullException("n");var m=new LsdMediaModel();m.SourcePath=path??"";m.ContainerType=n.Container??"Unknown";m.ContainerDetail=n.Detail??"";m.FileLength=n.Length;m.DurationSeconds=n.DurationSeconds;m.TimecodeScale=n.TimecodeScale;foreach(var x in n.Tracks){var t=new LsdTrackModel();t.TrackId=x.Number;t.TrackType=x.Kind;t.CodecId=x.CodecId??"";t.CodecFamily=Family(x.CodecId);t.Language=x.Language??"und";t.CodecPrivate=x.CodecPrivate;t.PayloadFormat=Payload(t.CodecFamily);t.NalLengthSize=x.NalLengthSize;t.Width=x.Width;t.Height=x.Height;t.Channels=x.AudioChannels>0?x.AudioChannels:x.Channels;t.SamplingRate=x.OutputSampleRate>0?x.OutputSampleRate:x.SamplingRate;t.PcmBitDepth=x.PcmBitDepth;t.PcmBlockAlign=x.PcmBlockAlign;t.PcmFloat=x.PcmFloat;t.PcmBigEndian=x.PcmBigEndian;m.Tracks.Add(t);}return m;}
...
public sealed class CanonicalAudioAnalysisResult {
 public string CodecFamily="Unknown",PayloadFormat="Unknown",FirstError="",DolbyCodec="",DolbyLayout="",PcmFormat=""; public long Samples,PayloadBytes,Minimum=Int64.MaxValue,Maximum,Rejected,MpegAudioFrames,MpegAudioBytes,MpegAudioPcmSamples,DolbyFrames,DolbyBytes,DolbyRejected,DolbyPcmSamples; public double DurationSeconds,Bitrate,Average,PcmDecodedDuration; public int PcmBitDepth,PcmBlockAlign,PcmChannels,PcmSampleRate; public bool PcmFloat,PcmBigEndian; public int MpegVersion,Layer,SampleRate,Channels,MinimumFrameBitrate=Int32.MaxValue,MaximumFrameBitrate,DolbySampleRate,DolbyChannels;
 public bool Complete{get{return Samples>0&&Rejected==0;}}
}
public static class CanonicalAudioAnalyzer {
 static int Bitrate(int version,int layer,int index){int[,] m1={{0,32,40,48,56,64,80,96,112,128,160,192,224,256,320,0},{0,32,48,56,64,80,96,112,128,160,192,224,256,320,384,0},{0,32,40,48,56,64,80,96,112,128,160,192,224,256,320,0}};int[,] m2={{0,8,16,24,32,40,48,56,64,80,96,112,128,144,160,0},{0,8,16,24,32,40,48,56,64,80,96,112,128,144,160,0},{0,32,40,48,56,64,80,96,112,128,144,160,176,192,224,0}};int row=layer==1?0:layer==2?1:2;return version==1?m1[row,index]:m2[row,index];}
 static int Rate(int version,int index){int[] a={44100,48000,32000,0};int r=a[index];if(version==2)r/=2;else if(version==25)r/=4;return r;}
 static int DolbyBits(byte[] b,ref int bit,int n){if(bit+n>b.Length*8)throw new System.IO.EndOfStreamException("Truncated Dolby syncframe header");int v=0;for(int i=0;i<n;i++){v=(v<<1)|((b[bit>>3]>>(7-(bit&7)))&1);bit++;}return v;}
 static int DolbyBaseChannels(int m){int[] c={2,1,2,3,3,4,4,5};return c[m&7];} static string DolbyLayoutName(int n){if(n==1)return "mono";if(n==2)return "stereo";if(n==6)return "5.1";if(n==8)return "7.1";return n+" channels";}
 static void Ac3(byte[] b,int p,int avail,CanonicalAudioAnalysisResult r,out int size){int bit=(p+4)*8;int f=DolbyBits(b,ref bit,2),fc=DolbyBits(b,ref bit,6),bsid=DolbyBits(b,ref bit,5);if(bsid>10)throw new System.IO.InvalidDataException("Not legacy AC-3");DolbyBits(b,ref bit,3);int acmod=DolbyBits(b,ref bit,3);if((acmod&1)!=0&&acmod!=1)DolbyBits(b,ref bit,2);if((acmod&4)!=0)DolbyBits(b,ref bit,2);if(acmod==2)DolbyBits(b,ref bit,2);bool lfe=DolbyBits(b,ref bit,1)!=0;int[] hzv={48000,44100,32000,0};int[] kv={32,40,48,56,64,80,96,112,128,160,192,224,256,320,384,448,512,576,640};int hz=hzv[f],code=fc>>1;if(hz==0||code>=kv.Length)throw new System.IO.InvalidDataException("Invalid AC-3 header");int kb=kv[code];if(hz==48000)size=kb*4;else if(hz==32000)size=kb*6;else{int[] w={69,87,104,121,139,174,208,243,278,348,417,487,557,696,835,975,1114,1253,1393};size=(w[code]+(fc&1))*2;}if(size>avail)throw new System.IO.EndOfStreamException("AC-3 syncframe exceeds sample");int ch=DolbyBaseChannels(acmod)+(lfe?1:0);r.DolbyPcmSamples+=1536;r.DolbyCodec="Dolby Digital / AC-3";r.DolbySampleRate=hz;r.DolbyChannels=ch;r.DolbyLayout=DolbyLayoutName(ch);}
 static void Eac3(byte[] b,int p,int avail,CanonicalAudioAnalysisResult r,out int size){int bit=(p+2)*8;DolbyBits(b,ref bit,2);DolbyBits(b,ref bit,3);size=(DolbyBits(b,ref bit,11)+1)*2;if(size>avail)throw new System.IO.EndOfStreamException("E-AC-3 syncframe exceeds sample");int f=DolbyBits(b,ref bit,2),hz,blocks;if(f==3){int f2=DolbyBits(b,ref bit,2);int[] h={24000,22050,16000,0};hz=h[f2];blocks=6;}else{int[] h={48000,44100,32000};hz=h[f];int[] bl={1,2,3,6};blocks=bl[DolbyBits(b,ref bit,2)];}int acmod=DolbyBits(b,ref bit,3);bool lfe=DolbyBits(b,ref bit,1)!=0;int bsid=DolbyBits(b,ref bit,5);if(bsid<=10)throw new System.IO.InvalidDataException("Not E-AC-3");int kb=(int)Math.Round(size*8.0*hz/(blocks*256.0)/1000.0);int ch=DolbyBaseChannels(acmod)+(lfe?1:0);r.DolbyPcmSamples+=(long)blocks*256;r.DolbyCodec="Dolby Digital Plus / E-AC-3";r.DolbySampleRate=hz;r.DolbyChannels=ch;r.DolbyLayout=DolbyLayoutName(ch);}
 static void ParseDolby(System.IO.FileStream fs,LsdSampleModel sample,CanonicalAudioAnalysisResult r){byte[] b=new byte[(int)sample.Length];fs.Position=sample.FileOffset;int o=0,n;while(o<b.Length&&(n=fs.Read(b,o,b.Length-o))>0)o+=n;if(o!=b.Length)throw new System.IO.EndOfStreamException("Truncated Dolby sample");int p=0,found=0;while(p+8<=b.Length){if(b[p]!=0x0B||b[p+1]!=0x77){p++;continue;}int size;try{int bsid=(b[p+5]>>3)&31;if(bsid<=10)Ac3(b,p,b.Length-p,r,out size);else Eac3(b,p,b.Length-p,r,out size);found++;r.DolbyFrames++;r.DolbyBytes+=size;p+=size;}catch(Exception){r.DolbyRejected++;throw;}}if(found==0)throw new System.IO.InvalidDataException("No Dolby syncframe found");}
 static void ParseMp3(System.IO.FileStream fs,LsdSampleModel sample,CanonicalAudioAnalysisResult r){byte[] b=new byte[(int)sample.Length];fs.Position=sample.FileOffset;int o=0,n;while(o<b.Length&&(n=fs.Read(b,o,b.Length-o))>0)o+=n;if(o!=b.Length)throw new System.IO.EndOfStreamException("Truncated MPEG Audio sample");int p=0,found=0;while(p+4<=b.Length){if(b[p]!=0xFF||(b[p+1]&0xE0)!=0xE0){p++;continue;}int vb=(b[p+1]>>3)&3,lb=(b[p+1]>>1)&3,bi=(b[p+2]>>4)&15,ri=(b[p+2]>>2)&3,pad=(b[p+2]>>1)&1;if(vb==1||lb==0||bi==0||bi==15||ri==3){p++;continue;}int version=vb==3?1:vb==2?2:25;int layer=4-lb;int kb=Bitrate(version,layer,bi),hz=Rate(version,ri);int size=layer==1?((12*kb*1000/hz)+pad)*4:(layer==3&&version!=1?72:144)*kb*1000/hz+pad;if(size<4||p+size>b.Length){p++;continue;}found++;r.MpegAudioFrames++;r.MpegAudioBytes+=size;r.MpegAudioPcmSamples+=layer==1?384:(layer==3&&version!=1?576:1152);r.MpegVersion=version;r.Layer=layer;r.SampleRate=hz;r.Channels=((b[p+3]>>6)&3)==3?1:2;if(kb<r.MinimumFrameBitrate)r.MinimumFrameBitrate=kb;if(kb>r.MaximumFrameBitrate)r.MaximumFrameBitrate=kb;p+=size;}if(found==0)throw new System.IO.InvalidDataException("No valid MPEG Audio frame header was found");}
 public static CanonicalAudioAnalysisResult Analyze(string path,LsdTrackModel track,double duration){var r=new CanonicalAudioAnalysisResult();if(track==null)return r;r.CodecFamily=track.CodecFamily;r.PayloadFormat=track.PayloadFormat;r.DurationSeconds=duration;using(var fs=new System.IO.FileStream(path,System.IO.FileMode.Open,System.IO.FileAccess.Read,System.IO.FileShare.ReadWrite)){foreach(var s in track.Samples){try{if(s.FileOffset<0||s.Length<=0||s.FileOffset+s.Length>fs.Length)throw new System.IO.InvalidDataException("Canonical audio sample is outside file bounds");r.Samples++;r.PayloadBytes+=s.Length;if(s.Length<r.Minimum)r.Minimum=s.Length;if(s.Length>r.Maximum)r.Maximum=s.Length;if(track.CodecFamily=="MP3")ParseMp3(fs,s,r);else if(track.CodecFamily=="AC3"||track.CodecFamily=="EAC3")ParseDolby(fs,s,r);else if(track.CodecFamily=="PCM"){if(track.PcmBlockAlign<=0||track.PcmBitDepth<=0||track.Channels<=0||track.SamplingRate<=0)throw new System.IO.InvalidDataException("Incomplete PCM format parameters");if((s.Length%track.PcmBlockAlign)!=0)throw new System.IO.InvalidDataException("PCM sample is not block aligned");r.PcmBitDepth=track.PcmBitDepth;r.PcmBlockAlign=track.PcmBlockAlign;r.PcmChannels=(int)track.Channels;r.PcmSampleRate=(int)track.SamplingRate;r.PcmFloat=track.PcmFloat;r.PcmBigEndian=track.PcmBigEndian;r.PcmFormat=(track.PcmFloat?"floating-point":"signed integer")+", "+track.PcmBitDepth+"-bit "+(track.PcmBigEndian?"big-endian":"little-endian");}}catch(Exception ex){r.Rejected++;if(r.FirstError.Length==0)r.FirstError=ex.GetType().Name+": "+ex.Message;}}}if(r.Samples>0){r.Average=(double)r.PayloadBytes/r.Samples;if(duration>0)r.Bitrate=r.PayloadBytes*8.0/duration;if(r.MpegAudioPcmSamples>0&&r.SampleRate>0)r.Bitrate=r.MpegAudioBytes*8.0/((double)r.MpegAudioPcmSamples/r.SampleRate);if(r.DolbyPcmSamples>0&&r.DolbySampleRate>0)r.Bitrate=r.DolbyBytes*8.0/((double)r.DolbyPcmSamples/r.DolbySampleRate);if(r.CodecFamily=="PCM"&&r.PcmBlockAlign>0&&r.PcmSampleRate>0){r.PcmDecodedDuration=(double)r.PayloadBytes/(r.PcmBlockAlign*r.PcmSampleRate);r.Bitrate=(double)r.PcmSampleRate*r.PcmBitDepth*r.PcmChannels;}}if(r.Minimum==Int64.MaxValue)r.Minimum=0;if(r.MinimumFrameBitrate==Int32.MaxValue)r.MinimumFrameBitrate=0;return r;}
}
...
public static class Av1SequenceHeaderProbe {
 static string PrimariesName(int v){switch(v){case 1:return "bt709";case 4:return "bt470m";case 5:return "bt470bg";case 6:return "smpte170m";case 7:return "smpte240m";case 8:return "film";case 9:return "bt2020";case 10:return "smpte428";case 11:return "smpte431";case 12:return "smpte432";case 22:return "ebu3213";default:return v>0?"code "+v:"";}}
 static string TransferName(int v){switch(v){case 1:return "bt709";case 4:return "gamma22";case 5:return "gamma28";case 6:return "smpte170m";case 7:return "smpte240m";case 8:return "linear";case 9:return "log";case 10:return "log_sqrt";case 11:return "iec61966-2-4";case 12:return "bt1361";case 13:return "iec61966-2-1";case 14:return "bt2020-10";case 15:return "bt2020-12";case 16:return "smpte2084";case 17:return "smpte428";case 18:return "arib-std-b67";default:return v>0?"code "+v:"";}}
 static string MatrixName(int v){switch(v){case 0:return "rgb";case 1:return "bt709";case 4:return "fcc";case 5:return "bt470bg";case 6:return "smpte170m";case 7:return "smpte240m";case 8:return "ycgco";case 9:return "bt2020nc";case 10:return "bt2020c";case 11:return "smpte2085";case 12:return "chroma-derived-nc";case 13:return "chroma-derived-c";case 14:return "ictcp";default:return v>0?"code "+v:"";}}
...
public static class NativeMp4Probe {
 ...
 static string Utf8Trim(byte[] b){return Encoding.UTF8.GetString(b).TrimEnd('\0').Trim();}
 static bool MetadataContainer(string t){return t=="udta"||t=="meta"||t=="ilst";}
 static string Mp4WritingApp(System.IO.Stream s,Mp4Box parent){long start=parent.Data;if(parent.Type=="meta")start+=4;if(start<0||start>parent.End)return "";foreach(var b in Children(s,start,parent.End)){if(b.Type=="?too"){foreach(var d in Children(s,b.Data,b.End))if(d.Type=="data"&&d.End-d.Data>=8){int n=(int)(d.End-d.Data-8);if(n>0&&n<1048576)return Utf8Trim(Bytes(s,d.Data+8,n));}}else if(b.Type=="?swr"&&b.End-b.Data>=4){s.Position=b.Data;int n=U16(s);U16(s);if(n>0&&n<=b.End-s.Position&&n<1048576)return Utf8Trim(Bytes(s,s.Position,n));}if(MetadataContainer(b.Type)){string value=Mp4WritingApp(s,b);if(value.Length>0)return value;}}return "";}
 ...
 static int EsdsObjectType(byte[] e){if(e==null)return -1;for(int i=0;i+2<e.Length;i++)if(e[i]==4){int p=i+1,n=0,z;do{if(p>=e.Length)return -1;z=e[p++];n=(n<<7)|(z&127);}while((z&128)!=0);if(n>0&&p<e.Length&&p+n<=e.Length)return e[p];}return -1;}
 ...
 static void Stsd(System.IO.Stream s,Mp4Box b,Mp4TrackData t){s.Position=b.Data+4;uint count=U32(s);if(count==0)return;long entryStart=s.Position;uint size=U32(s);string four=Four(s);long payload=s.Position,end=entryStart+size;if(size<8||end>b.End)throw new System.IO.InvalidDataException("Invalid stsd entry");t.CodecFourcc=four;var n=new NativeTrack();n.Number=t.TrackId;n.Language="und";if(t.Handler=="vide"){
   n.Type=1;s.Position=payload+24;n.Width=U16(s);n.Height=U16(s);long child=payload+78;if(four=="avc1"||four=="avc3"){n.CodecId="V_MPEG4/ISO/AVC";n.CodecPrivate=FindConfig(s,child,end,"avcC");}else if(four=="hvc1"||four=="hev1"){n.CodecId="V_MPEGH/ISO/HEVC";n.CodecPrivate=FindConfig(s,child,end,"hvcC");}else if(four=="av01"){n.CodecId="V_AV1";n.CodecPrivate=FindConfig(s,child,end,"av1C");if(n.CodecPrivate==null||n.CodecPrivate.Length<4)throw new System.IO.InvalidDataException("MP4 av01 sample entry has no valid av1C configuration box");}else if(four=="mp4v"){n.CodecId="V_MPEG4/ISO/ASP";}else n.CodecId="V_"+four.ToUpperInvariant();
  }else if(t.Handler=="soun"){n.Type=2;s.Position=payload+8;int soundVersion=U16(s);s.Position=payload+16;n.Channels=U16(s);s.Position=payload+24;n.SamplingRate=U32(s)/65536.0;long child=payload+(soundVersion==1?44:soundVersion==2?64:28);if(child>end)throw new System.IO.InvalidDataException("Invalid QuickTime audio sample entry version");if(four=="mp4a"){byte[] esds=FindConfig(s,child,end,"esds");int oti=EsdsObjectType(esds);if(oti==0x69||oti==0x6B){n.CodecId="A_MPEG/L3";n.CodecPrivate=esds;}else{n.CodecId="A_AAC";n.CodecPrivate=AscFromEsds(esds);}}else if(four==".mp3"||four=="mp3 "){n.CodecId="A_MPEG/L3";}else if(four=="ac-3"){n.CodecId="A_AC3";n.CodecPrivate=FindConfig(s,child,end,"dac3");}else if(four=="ec-3"){n.CodecId="A_EAC3";n.CodecPrivate=FindConfig(s,child,end,"dec3");}else if(four=="sowt"||four=="twos"||four=="in24"||four=="in32"||four=="fl32"||four=="fl64"||four=="lpcm"){n.CodecId="A_PCM";n.PcmFloat=four=="fl32"||four=="fl64";n.PcmBigEndian=four=="twos";n.PcmBitDepth=four=="in24"?24:four=="in32"||four=="fl32"?32:four=="fl64"?64:16;n.PcmBlockAlign=(int)n.Channels*((n.PcmBitDepth+7)/8);n.PcmFormat=four;}else n.CodecId="A_"+four.ToUpperInvariant();}else{n.Type=17;n.CodecId=four;}t.Native=n;}
 ...
 static Mp4TrackData Track(System.IO.Stream s,Mp4Box trak){var t=new Mp4TrackData();var tk=Path(s,trak,"tkhd");if(tk!=null)Tkhd(s,tk,t);var md=Path(s,trak,"mdia","mdhd");if(md!=null)Mdhd(s,md,t);var hd=Path(s,trak,"mdia","hdlr");if(hd!=null){s.Position=hd.Data+8;t.Handler=Four(s);}var stbl=Path(s,trak,"mdia","minf","stbl");if(stbl==null)return t;foreach(var b in Children(s,stbl.Data,stbl.End))if(b.Type=="stsd")Stsd(s,b,t);if(t.Handler=="soun"){foreach(var b in Children(s,stbl.Data,stbl.End)){if(b.Type=="stsz")Stsz(s,b,t);else if(b.Type=="stco")Stco(s,b,t,false);else if(b.Type=="co64")Stco(s,b,t,true);else if(b.Type=="stsc")Stsc(s,b,t);else if(b.Type=="stts")Stts(s,b,t);}return t;}if(t.Handler!="vide")return t;foreach(var b in Children(s,stbl.Data,stbl.End)){if(b.Type=="stsz")Stsz(s,b,t);else if(b.Type=="stco")Stco(s,b,t,false);else if(b.Type=="co64")Stco(s,b,t,true);else if(b.Type=="stsc")Stsc(s,b,t);else if(b.Type=="stts")Stts(s,b,t);else if(b.Type=="ctts")Ctts(s,b,t);else if(b.Type=="stss")Stss(s,b,t);}return t;}
 static void BuildSamples(Mp4TrackData t,LsdTrackModel lt){if(t.Sizes.Count==0||t.Chunks.Count==0||t.Stsc.Count==0)return;if(lt.Samples.Capacity<t.Sizes.Count)lt.Samples.Capacity=t.Sizes.Count;int sample=0,entry=0;for(int ci=0;ci<t.Chunks.Count&&sample<t.Sizes.Count;ci++){while(entry+1<t.Stsc.Count&&t.Stsc[entry+1].FirstChunk<=ci+1)entry++;uint per=t.Stsc[entry].SamplesPerChunk;long off=(long)t.Chunks[ci];for(uint j=0;j<per&&sample<t.Sizes.Count;j++,sample++){uint len=t.Sizes[sample];long dts=sample<t.Dts.Count?t.Dts[sample]:0;long pts=dts+(sample<t.Pts.Count?t.Pts[sample]:0);lt.Samples.Add(new LsdSampleModel{TrackId=lt.TrackId,FileOffset=off,Length=len,DecodeTimestamp=dts,PresentationTimestamp=pts,IsKeyFrame=lt.TrackType=="audio"||!t.HasSync||t.Sync.Contains(sample),DescriptionIndex=(int)t.Stsc[entry].DescriptionIndex,PayloadFormat=lt.PayloadFormat});off+=len;}}if(sample!=t.Sizes.Count)throw new System.IO.InvalidDataException("MP4 canonical sample/chunk table is incomplete");}
 public static Mp4ProbeResult Inspect(string path){var result=new Mp4ProbeResult();var n=new NativeMediaInfo();var fi=new System.IO.FileInfo(path);n.Length=fi.Length;n.ModifiedUtc=fi.LastWriteTimeUtc;n.Container="MP4 / MOV";n.Detail="ISO Base Media";n.DocType="mp4";result.Info=n;using(var s=new System.IO.FileStream(path,System.IO.FileMode.Open,System.IO.FileAccess.Read,System.IO.FileShare.ReadWrite)){Mp4Box moov=null;foreach(var b in Children(s,0,s.Length))if(b.Type=="moov"){moov=b;break;}if(moov==null)throw new System.IO.InvalidDataException("MP4 moov box was not found");n.AssignSoftware(Mp4WritingApp(s,moov));foreach(var b in Children(s,moov.Data,moov.End))if(b.Type=="trak"){var t=Track(s,b);if(t.Native!=null){t.Native.Number=t.TrackId;ParsePrivate(t.Native);n.Tracks.Add(t.Native);result.Tracks.Add(t);if(t.TimeScale>0){double d=(double)t.Duration/t.TimeScale;if(t.Native.Type==1&&d>0)t.Native.FrameRate=t.Sizes.Count/d;if(t.Native.Type==2&&d>0){long mp4AudioBytes=0;foreach(uint sampleSize in t.Sizes)mp4AudioBytes+=sampleSize;t.Native.AudioPayloadBytes=mp4AudioBytes;t.Native.AudioBitrate=mp4AudioBytes*8.0/d;}if(d>n.DurationSeconds)n.DurationSeconds=d;}}}}return result;}
 static void ParsePrivate(NativeTrack t){NativeMediaProbe.PopulateCodecPrivate(t);}

 public static LsdMediaModel Adapt(string path,Mp4ProbeResult p){var m=LsdMatroskaAdapter.Adapt(path,p.Info);m.ContainerType="MP4 / MOV";m.ContainerDetail="ISO Base Media";m.AdapterName="Mp4ToLsdAdapter/2.0";for(int i=0;i<p.Tracks.Count;i++){var t=p.Tracks[i];LsdTrackModel lt=null;foreach(var q in m.Tracks)if(q.TrackId==t.TrackId){lt=q;break;}if(lt!=null){BuildSamples(t,lt);if(lt.TrackType=="video"&&t.TimeScale>0)m.TimecodeScale=(long)Math.Round(1000000000.0/t.TimeScale);}}return m;}
}


// RIFF AVI 1.0 and OpenDML AVI 2.0 reader with canonical sample preparation.
public sealed class AviStreamData {
 public int Index; public string Type="",Handler=""; public uint Scale,Rate,Length; public NativeTrack Native;
}
public sealed class AviProbeResult {
 public NativeMediaInfo Info; public long MoviData; public string IndexMode="None"; public int RiffSegments,AvixSegments,StandardIndexes; public bool LegacyIndexPresent,OpenDml;
 public readonly System.Collections.Generic.List<AviStreamData> Streams=new System.Collections.Generic.List<AviStreamData>();
 public readonly System.Collections.Generic.List<LsdSampleModel> VideoSamples=new System.Collections.Generic.List<LsdSampleModel>(); public readonly System.Collections.Generic.List<LsdSampleModel> AudioSamples=new System.Collections.Generic.List<LsdSampleModel>();
 public readonly System.Collections.Generic.List<long> StandardIndexPositions=new System.Collections.Generic.List<long>();
}
public static class NativeAviProbe {
 static ushort U16(System.IO.Stream s){int a=s.ReadByte(),b=s.ReadByte();if(b<0)throw new System.IO.EndOfStreamException();return (ushort)(a|(b<<8));}
 static uint U32(System.IO.Stream s){int a=s.ReadByte(),b=s.ReadByte(),c=s.ReadByte(),d=s.ReadByte();if(d<0)throw new System.IO.EndOfStreamException();return (uint)(a|(b<<8)|(c<<16)|(d<<24));}
 static ulong U64(System.IO.Stream s){return (ulong)U32(s)|((ulong)U32(s)<<32);}
 static string Four(System.IO.Stream s){byte[] b=new byte[4];if(s.Read(b,0,4)!=4)throw new System.IO.EndOfStreamException();return Encoding.ASCII.GetString(b);}
 static uint AtU32(System.IO.Stream s,long p){s.Position=p;return U32(s);} static string AtFour(System.IO.Stream s,long p){s.Position=p;return Four(s);}
 static string InfoText(System.IO.Stream s,long p,uint n){if(n==0||n>1048576)return "";byte[] b=new byte[n];s.Position=p;int o=0,k;while(o<b.Length&&(k=s.Read(b,o,b.Length-o))>0)o+=k;return Encoding.UTF8.GetString(b,0,o).TrimEnd('\0',' ','\r','\n');}
 static void ParseInfo(System.IO.Stream s,long start,long end,NativeMediaInfo n){long p=start;while(p+8<=end){s.Position=p;string id=Four(s);uint z=U32(s);long data=p+8,next=data+z+(z&1);if(next>end||next<data)break;if(id=="ISFT")n.AssignSoftware(InfoText(s,data,z));p=next;}}
 static string VideoCodec(string f){f=(f??"").ToUpperInvariant();if(f=="XVID"||f=="DIVX"||f=="DX50"||f=="FMP4"||f=="MP4V")return "V_MPEG4/ISO/ASP";if(f=="H264"||f=="X264"||f=="AVC1"||f=="DAVC")return "V_MPEG4/ISO/AVC";return "V_"+f;}
 static string AudioCodec(ushort tag){if(tag==1||tag==3||tag==0xFFFE)return "A_PCM";if(tag==0x55)return "A_MPEG/L3";if(tag==0x2000)return "A_AC3";return "A_WAVEFORMAT_"+tag.ToString("X4");}
 static void ParseStrl(System.IO.Stream s,long start,long end,AviProbeResult r){var a=new AviStreamData();a.Index=r.Streams.Count;long p=start;byte[] strf=null;while(p+8<=end){s.Position=p;string id=Four(s);uint z=U32(s);long data=p+8,next=data+z+(z&1);if(next>end||next<data)break;if(id=="strh"&&z>=48){s.Position=data;a.Type=Four(s);a.Handler=Four(s);s.Position=data+20;a.Scale=U32(s);a.Rate=U32(s);s.Position=data+32;a.Length=U32(s);}else if(id=="strf"&&z>0&&z<1048576){strf=new byte[z];s.Position=data;if(s.Read(strf,0,(int)z)!=(int)z)throw new System.IO.EndOfStreamException();}p=next;}
  var n=new NativeTrack();n.Number=a.Index;n.Language="und";if(a.Type=="vids"){n.Type=1;n.CodecId=VideoCodec(a.Handler);if(strf!=null&&strf.Length>=40){n.Width=BitConverter.ToInt32(strf,4);n.Height=Math.Abs(BitConverter.ToInt32(strf,8));string comp=Encoding.ASCII.GetString(strf,16,4);if(a.Handler.Length==0||a.Handler=="\0\0\0\0")a.Handler=comp;}if(a.Scale>0)n.FrameRate=(double)a.Rate/a.Scale;n.Chroma="YUV 4:2:0";n.BitDepth=8;}else if(a.Type=="auds"){n.Type=2;if(strf!=null&&strf.Length>=16){ushort tag=BitConverter.ToUInt16(strf,0);n.CodecId=AudioCodec(tag);n.Channels=BitConverter.ToUInt16(strf,2);n.SamplingRate=BitConverter.ToUInt32(strf,4);n.PcmBlockAlign=BitConverter.ToUInt16(strf,12);if(strf.Length>=16)n.PcmBitDepth=BitConverter.ToUInt16(strf,14);n.PcmFloat=tag==3;n.PcmBigEndian=false;if(tag==0xFFFE&&strf.Length>=40){byte sub=strf[24];n.PcmFloat=sub==3;}if(n.CodecId=="A_PCM"){if(n.PcmBitDepth<=0&&n.Channels>0&&n.PcmBlockAlign>0)n.PcmBitDepth=n.PcmBlockAlign*8/(int)n.Channels;n.PcmFormat=(n.PcmFloat?"IEEE float":"signed integer")+", little-endian";}n.AudioChannels=(int)n.Channels;n.CoreAudioChannels=(int)n.Channels;n.CoreSampleRate=(int)n.SamplingRate;n.OutputSampleRate=(int)n.SamplingRate;n.OutputSamplingRate=n.SamplingRate;}}else{n.Type=17;n.CodecId=a.Handler;}a.Native=n;r.Streams.Add(a);r.Info.Tracks.Add(n);if(a.Rate>0){double d=(double)a.Length*a.Scale/a.Rate;if(d>r.Info.DurationSeconds)r.Info.DurationSeconds=d;}}
 static void ParseHeaderList(System.IO.Stream s,long start,long end,AviProbeResult r){long q=start;while(q+8<=end){s.Position=q;string id=Four(s);uint z=U32(s);long data=q+8,next=data+z+(z&1);if(next>end||next<data)break;if(id=="LIST"&&z>=4&&AtFour(s,data)=="strl")ParseStrl(s,data+4,next,r);q=next;}}
 static void PopulateAviAvc(System.IO.FileStream s,AviProbeResult r){foreach(var stream in r.Streams){if(stream.Native==null||stream.Native.Type!=1||stream.Native.CodecId!="V_MPEG4/ISO/AVC")continue;var bytes=new System.Collections.Generic.List<byte>();foreach(var sample in r.VideoSamples){if(sample.TrackId!=stream.Native.Number||sample.Length<=0||sample.Length>16*1024*1024)continue;byte[] z=new byte[(int)sample.Length];s.Position=sample.FileOffset;int o=0,k;while(o<z.Length&&(k=s.Read(z,o,z.Length-o))>0)o+=k;if(o==z.Length)bytes.AddRange(z);if(stream.Native.Profile.Length>0)break;if(bytes.Count>4*1024*1024)break;}NativeMediaProbe.PopulateAvcAnnexB(stream.Native,bytes.ToArray());}}
 static long ResolveChunk(System.IO.Stream s,long moviData,long fileLength,string id,uint off,uint len){long[] c={moviData-4+(long)off,moviData+(long)off,(long)off,(long)off+8};foreach(long p in c){if(p>=0&&p+8+(long)len<=fileLength){try{if(AtFour(s,p)==id&&AtU32(s,p+4)>=len)return p+8;}catch{}}}return -1;}
 static long ResolveOpenDml(System.IO.Stream s,long fileLength,string id,ulong baseOffset,uint offset,uint len){ulong raw=baseOffset+offset;if(raw>Int64.MaxValue)return -1;long a=(long)raw;long[] c={a,a-8,a+8};foreach(long p in c){if(p>=0&&p+8+(long)len<=fileLength){try{if(AtFour(s,p)==id&&AtU32(s,p+4)>=len)return p+8;}catch{}}if(p>=0&&p+(long)len<=fileLength&&p>=8){try{if(AtFour(s,p-8)==id&&AtU32(s,p-4)>=len)return p;}catch{}}}return -1;}
 static void AddSample(AviProbeResult r,int si,long payload,uint len,bool key,long[] counters){if(si<0||si>=r.Streams.Count||len==0)return;var st=r.Streams[si];long k=counters[si]++;if(st.Native.Type==1)r.VideoSamples.Add(new LsdSampleModel{TrackId=st.Native.Number,FileOffset=payload,Length=len,DecodeTimestamp=k,PresentationTimestamp=k,IsKeyFrame=key,PayloadFormat=st.Native.CodecId=="V_MPEG4/ISO/AVC"?"AnnexBNal":"Mpeg4Visual"});else if(st.Native.Type==2){st.Native.AudioPayloadBytes+=len;r.AudioSamples.Add(new LsdSampleModel{TrackId=st.Native.Number,FileOffset=payload,Length=len,DecodeTimestamp=k,PresentationTimestamp=k,IsKeyFrame=true,PayloadFormat=st.Native.CodecId=="A_MPEG/L3"?"MpegAudio":"RawCodecFrame"});}}
 static int StreamNumber(string id){int n=-1;if(id!=null&&id.Length>=2)Int32.TryParse(id.Substring(0,2),out n);return n;}
 static int ParseStandardIndex(System.IO.FileStream s,long pos,AviProbeResult r,long[] counters,System.Collections.Generic.HashSet<long> seen){s.Position=pos;string ix=Four(s);uint chunkSize=U32(s);long end=pos+8+chunkSize;if(!ix.StartsWith("ix")||end>s.Length||chunkSize<24)return 0;ushort longs=U16(s);int subtype=s.ReadByte(),type=s.ReadByte();uint entries=U32(s);string chunkId=Four(s);ulong baseOffset=U64(s);U32(s);if(longs!=2||type!=1||subtype!=0||entries>(chunkSize-24)/8)return 0;int si=StreamNumber(chunkId),added=0;for(uint i=0;i<entries;i++){uint off=U32(s),raw=U32(s),len=raw&0x7FFFFFFF;if(len==0)continue;bool key=(raw&0x80000000)==0;long payload=ResolveOpenDml(s,s.Length,chunkId,baseOffset,off,len);s.Position=pos+8+24+(i+1)*8;if(payload<0)throw new System.IO.InvalidDataException("OpenDML index entry cannot be resolved: "+chunkId+" @ "+off);if(seen.Add(payload)){AddSample(r,si,payload,len,key,counters);added++;}}return added;}
 static void ScanRiff(System.IO.FileStream s,long start,AviProbeResult r,ref long idx,ref long idxSize){s.Position=start;if(Four(s)!="RIFF")throw new System.IO.InvalidDataException("Expected RIFF segment");uint z=U32(s);string form=Four(s);long end=Math.Min(s.Length,start+8+(long)z);if(end<start+12)throw new System.IO.InvalidDataException("Invalid RIFF segment");r.RiffSegments++;if(form=="AVIX"){r.AvixSegments++;r.OpenDml=true;}else if(form!="AVI ")throw new System.IO.InvalidDataException("Unsupported RIFF form "+form);long p=start+12;while(p+8<=end){s.Position=p;string id=Four(s);uint size=U32(s);long data=p+8,next=data+size+(size&1);if(next>end||next<data)throw new System.IO.InvalidDataException("Invalid AVI chunk "+id);if(id=="LIST"&&size>=4){string list=AtFour(s,data);if(list=="hdrl"&&r.Streams.Count==0)ParseHeaderList(s,data+4,next,r);else if(list=="movi"&&r.MoviData==0)r.MoviData=data+4;else if(list=="INFO")ParseInfo(s,data+4,next,r.Info);}else if(id=="idx1"){r.LegacyIndexPresent=true;idx=data;idxSize=size;}else if(id.Length==4&&id[0]=='i'&&id[1]=='x'){r.StandardIndexPositions.Add(p);r.OpenDml=true;}p=next;}}
 public static AviProbeResult Inspect(string path){var r=new AviProbeResult();var n=new NativeMediaInfo();var fi=new System.IO.FileInfo(path);n.Length=fi.Length;n.ModifiedUtc=fi.LastWriteTimeUtc;n.Container="AVI";n.Detail="RIFF AVI";n.DocType="avi";r.Info=n;using(var s=new System.IO.FileStream(path,System.IO.FileMode.Open,System.IO.FileAccess.Read,System.IO.FileShare.ReadWrite)){long idx=-1,idxSize=0,p=0;while(p+12<=s.Length){s.Position=p;if(Four(s)!="RIFF")throw new System.IO.InvalidDataException("AVI top-level RIFF segment was not found at "+p);uint z=U32(s);long next=p+8+(long)z+(z&1);if(next<=p||next>s.Length)throw new System.IO.InvalidDataException("Invalid AVI RIFF segment size");ScanRiff(s,p,r,ref idx,ref idxSize);p=next;}if(r.Streams.Count==0||r.MoviData<=0)throw new System.IO.InvalidDataException("AVI headers or movi list were not found");long[] counters=new long[Math.Max(1,r.Streams.Count)];var seen=new System.Collections.Generic.HashSet<long>();int openDmlSamples=0;foreach(long ix in r.StandardIndexPositions){openDmlSamples+=ParseStandardIndex(s,ix,r,counters,seen);r.StandardIndexes++;}if(openDmlSamples>0){r.IndexMode="OpenDML standard indexes";r.OpenDml=true;}else{if(idx<0)throw new System.IO.InvalidDataException("No valid OpenDML or idx1 index was found");s.Position=idx;long entries=idxSize/16;for(long i=0;i<entries;i++){string cid=Four(s);uint flags=U32(s),off=U32(s),len=U32(s);if(len==0)continue;long payload=ResolveChunk(s,r.MoviData,s.Length,cid,off,len);s.Position=idx+(i+1)*16;if(payload<0)throw new System.IO.InvalidDataException("AVI idx1 entry cannot be resolved: "+cid+" @ "+off);AddSample(r,StreamNumber(cid),payload,len,(flags&0x10)!=0,counters);}r.IndexMode="AVI 1.0 idx1";}n.Detail=r.OpenDml?"OpenDML AVI 2.0":"RIFF AVI 1.0";PopulateAviAvc(s,r);foreach(var a in r.Streams)if(a.Native!=null&&a.Native.Type==2){if(a.Native.CodecId=="A_PCM"&&a.Native.PcmBitDepth>0&&a.Native.SamplingRate>0&&a.Native.Channels>0)a.Native.AudioBitrate=a.Native.SamplingRate*a.Native.PcmBitDepth*a.Native.Channels;else if(a.Rate>0){double seconds=(double)a.Length*a.Scale/a.Rate;if(seconds>0)a.Native.AudioBitrate=a.Native.AudioPayloadBytes*8.0/seconds;}}return r;}}
 public static LsdMediaModel Adapt(string path,AviProbeResult p){var m=LsdMatroskaAdapter.Adapt(path,p.Info);m.ContainerType="AVI";m.ContainerDetail=p.Info.Detail;m.AdapterName="AviToLsdAdapter/2.0";m.TimecodeScale=1000000;foreach(var t in m.Tracks){if(t.TrackType=="video"){t.PayloadFormat=t.CodecFamily=="MPEG4ASP"?"Mpeg4Visual":t.CodecFamily=="AVC"?"AnnexBNal":t.PayloadFormat;foreach(var x in p.VideoSamples)if(x.TrackId==t.TrackId){var st=p.Streams[(int)t.TrackId];long ns=st.Rate>0?(long)Math.Round(1000000000.0*st.Scale/st.Rate):1000000;m.TimecodeScale=ns;t.Samples.Add(x);}}else if(t.TrackType=="audio")foreach(var x in p.AudioSamples)if(x.TrackId==t.TrackId)t.Samples.Add(x);}return m;}
}
public sealed class NativeAviPreparationJob : IDisposable {
 System.Threading.Thread thread; volatile bool done,cancelled; string error=""; AviProbeResult probe; LsdMediaModel media;
 public bool Done{get{return done;}} public bool Cancelled{get{return cancelled;}} public string Error{get{return error;}} public AviProbeResult Probe{get{return probe;}} public LsdMediaModel Media{get{return media;}}
 public void Start(string path){thread=new System.Threading.Thread(()=>Run(path));thread.IsBackground=true;thread.Name="LSD AVI index preparation";thread.Start();}
 void Run(string path){try{if(cancelled)return;var p=NativeAviProbe.Inspect(path);if(cancelled)return;var m=NativeAviProbe.Adapt(path,p);if(cancelled)return;probe=p;media=m;}catch(Exception ex){error=ex.ToString();}finally{done=true;}}
...
public static class NativeMediaProbe {
 ...
 public static void PopulateAvcAnnexB(NativeTrack t,byte[] data){if(t==null||data==null)return;int p=0;while(p+4<=data.Length){int sc=0;if(p+3<=data.Length&&data[p]==0&&data[p+1]==0&&data[p+2]==1)sc=3;else if(p+4<=data.Length&&data[p]==0&&data[p+1]==0&&data[p+2]==0&&data[p+3]==1)sc=4;if(sc==0){p++;continue;}int start=p+sc,end=start;while(end+3<data.Length&&!(data[end]==0&&data[end+1]==0&&(data[end+2]==1||(data[end+2]==0&&data[end+3]==1))))end++;if(end<=start){p=start;continue;}byte[] nal=new byte[end-start];Array.Copy(data,start,nal,0,nal.Length);int type=nal[0]&31;if(type==7)ParseSps(nal,t);else if(type==8)ParsePps(nal,t);p=end;}t.NalLengthSize=4;}
 public static void PopulateCodecPrivate(NativeTrack t){if(t==null)throw new ArgumentNullException("t");ParsePrivate(t);}
  static void ParsePrivate(NativeTrack t){if(t.OutputSamplingRate<=0)t.OutputSamplingRate=t.SamplingRate;if(t.CodecId!=null&&t.CodecId.StartsWith("A_PCM")){if(t.CodecId=="A_PCM/FLOAT/IEEE")t.PcmFloat=true;if(t.CodecId=="A_PCM/INT/BIG")t.PcmBigEndian=true;if(t.PcmBitDepth<=0)t.PcmBitDepth=t.PcmFloat?32:16;if(t.PcmBlockAlign<=0)t.PcmBlockAlign=(int)t.Channels*((t.PcmBitDepth+7)/8);if(t.PcmFormat.Length==0)t.PcmFormat=t.CodecId;t.CoreSampleRate=(int)t.SamplingRate;t.OutputSampleRate=(int)t.OutputSamplingRate;t.CoreAudioChannels=(int)t.Channels;t.AudioChannels=(int)t.Channels;}try{if(t.CodecId=="V_MPEG4/ISO/AVC")ParseAvc(t.CodecPrivate,t);else if(t.CodecId=="V_MPEGH/ISO/HEVC")ParseHevc(t.CodecPrivate,t);else if(t.CodecId=="V_AV1")Av1CodecConfigurationProbe.Populate(t);else if(t.CodecId=="A_AAC"){ParseAac(t.CodecPrivate,t);if(!t.Ps&&t.Channels>0){t.CoreAudioChannels=(int)t.Channels;t.AudioChannels=(int)t.Channels;}else if(t.CoreAudioChannels<=0&&t.Channels>0)t.CoreAudioChannels=(int)t.Channels;if(t.OutputSamplingRate>0)t.OutputSampleRate=(int)t.OutputSamplingRate;}}catch{}}
 ...
 public static NativeMediaInfo Inspect(string path){
...
readonly double duration; readonly int nalLengthSize; readonly H264SliceHeaderConfig sliceConfig; readonly bool avcMode,hevcMode,av1Mode,mpeg4Mode; readonly LsdTrackModel readyTrack,readyAudioTrack; readonly long readyTimecodeScale; readonly NativeTrack sourceTrack,audioSourceTrack; readonly int hevcExtraSliceHeaderBits; readonly HevcHvccProbeResult hevcProbeContext; long hevcPrefixParsed,hevcPrefixRejected; string hevcPrefixFirstError=""; readonly long[] bins=new long[240]; readonly long[] qpHistogram=new long[64]; readonly StringBuilder hevcTypeSequence=new StringBuilder();
 ...
 sealed class M4Bits { readonly byte[] b; int p; public M4Bits(byte[] x,int bit){b=x;p=bit;} public int Bit(){if(p>=b.Length*8)throw new System.IO.EndOfStreamException("MPEG-4 VOP ended");return (b[p>>3]>>(7-(p++&7)))&1;} public int Bits(int n){int v=0;while(n-->0)v=(v<<1)|Bit();return v;} }
 int ParseMpeg4Payload(FastReader r,long length){if(length<=0||length>Int32.MaxValue)throw new System.IO.InvalidDataException("Invalid MPEG-4 sample length");byte[] d=r.ReadSmall((int)length);int first=-1,vops=0;for(int p=0;p+5<d.Length;p++){if(d[p]==0&&d[p+1]==0&&d[p+2]==1&&d[p+3]==0xB6){vops++;try{var b=new M4Bits(d,(p+4)*8);int type=b.Bits(2);while(b.Bit()!=0){}b.Bit();b.Bits(5);b.Bit();bool coded=b.Bit()!=0;if(!coded){mpeg4NVop++;continue;}if(type==1||type==3)b.Bit();b.Bits(3);int q=b.Bits(5);if(q<1||q>31)throw new System.IO.InvalidDataException("MPEG-4 VOP quantizer outside 1..31");mpeg4VopParsed++;mpeg4QuantHistogram[q]++;mpeg4QuantSum+=q;mpeg4QuantSquares+=(long)q*q;if(q<mpeg4QuantMin)mpeg4QuantMin=q;if(q>mpeg4QuantMax)mpeg4QuantMax=q;int st=type==0?2:type==1?0:type==2?1:-1;if(first<0)first=st;}catch(Exception ex){mpeg4VopRejected++;if(mpeg4FirstError.Length==0)mpeg4FirstError=ex.GetType().Name+": "+ex.Message;}}}if(vops>1)mpeg4PackedSamples++;return first;}
 int ParseAnnexBPayload(FastReader r,long length){if(length<=0||length>Int32.MaxValue)throw new System.IO.InvalidDataException("Invalid Annex B sample length");byte[] d=r.ReadSmall((int)length);int first=-1,p=0;while(p+4<=d.Length){int sc=0;if(p+3<=d.Length&&d[p]==0&&d[p+1]==0&&d[p+2]==1)sc=3;else if(p+4<=d.Length&&d[p]==0&&d[p+1]==0&&d[p+2]==0&&d[p+3]==1)sc=4;if(sc==0){p++;continue;}int start=p+sc,end=start;while(end+3<d.Length&&!(d[end]==0&&d[end+1]==0&&(d[end+2]==1||(d[end+2]==0&&d[end+3]==1))))end++;if(end>start){byte[] nal=new byte[end-start];Array.Copy(d,start,nal,0,nal.Length);if(first<0)first=SliceType(nal);}p=end;}return first;}
 int ParseVideoPayload(FastReader r,long length){if(avcMode&&readyTrack!=null&&readyTrack.PayloadFormat=="AnnexBNal")return ParseAnnexBPayload(r,length);if(mpeg4Mode)return ParseMpeg4Payload(r,length);if(av1Mode)return ParseAv1Payload(r,length);long end=r.Position+length;int found=-1;while(r.Position+nalLengthSize<=end){long n=0;for(int i=0;i<nalLengthSize;i++){int v=r.ReadByteFast();if(v<0)return found;n=(n<<8)|(byte)v;}if(n<=0||r.Position+n>end){r.SkipTo(end);break;}int take=(int)Math.Min(n,8192);byte[] head=r.ReadSmall(take);CountHevcNal(head);if(found<0){if(hevcMode)found=HevcSliceType(head);else found=SliceType(head);}r.SkipTo(r.Position+(n-take));}r.SkipTo(end);return found;}
 ...
 void Block(FastReader r,long end,long target,long clusterTc,bool simple){
  ...
  bool isVideo=track==target;bool isAudio=audioSourceTrack!=null&&track==audioSourceTrack.Number;if(!isVideo&&!isAudio){r.SkipTo(end);return;}
  if(lace==0){long off=r.Position;if(isVideo){RecordSample(off,remain,rawTime,keyFlag,flags);int st=ParseVideoPayload(r,remain);AddFrame(remain,keyFlag,sec);AddSlice(st);}else{audioSourceTrack.AudioPayloadBytes+=remain;if(readyAudioTrack!=null)readyAudioTrack.Samples.Add(new LsdSampleModel{TrackId=track,FileOffset=off,Length=remain,DecodeTimestamp=rawTime,PresentationTimestamp=rawTime,IsKeyFrame=true,PayloadFormat=readyAudioTrack.PayloadFormat});r.SkipTo(end);}return;}
  int firstCount=r.ReadByteFast();if(firstCount<0)throw new System.IO.EndOfStreamException();int frames=firstCount+1;var sizes=new long[frames];
  ...
  for(int i=0;i<frames;i++){long len=sizes[i];if(len<0||r.Position+len>end)throw new System.IO.InvalidDataException("Invalid Matroska lace size");long off=r.Position;if(isVideo){RecordSample(off,len,rawTime,keyFlag,flags);int st=ParseVideoPayload(r,len);AddFrame(len,keyFlag,sec);AddSlice(st);}else{audioSourceTrack.AudioPayloadBytes+=len;if(readyAudioTrack!=null)readyAudioTrack.Samples.Add(new LsdSampleModel{TrackId=track,FileOffset=off,Length=len,DecodeTimestamp=rawTime,PresentationTimestamp=rawTime,IsKeyFrame=true,PayloadFormat=readyAudioTrack.PayloadFormat});r.SkipTo(off+len);}}
  r.SkipTo(end);
 }
...
$Version='2.0';$script:Build='2.0 - Final stable canonical media pipeline';$script:ScanJob=$null;$script:ScanPhase='idle';$script:Cancelled=$false;$script:File='';$script:Meta=$null;$script:NativeInfo=$null;$script:ReadyMedia=$null;$script:ReadyTrack=$null;$script:ReadyModelStatus='Not built';$script:Mp4Probe=$null;$script:AviProbe=$null;$script:Mp4PrepJob=$null;$script:PacketEngine='';$script:Duration=0;$script:PacketResult=$null;$script:FrameResult=$null;$script:BitrateBins=@();$script:QpCounts=@{};$script:QpMode='SliceQPY';$script:QpDecoderState=New-Object H264QpValidationState;$script:SliceQpDiagnostic=$null;$script:HevcDiagnostic=$null;$script:Av1Diagnostic=$null;$script:Mpeg4Diagnostic=$null;$script:AudioDiagnostic=$null;$script:NativeHevcQpDiagnostic=$null;$script:StageStarted=[DateTime]::MinValue
...
function Render-QpHistogram {
    ...
    $isAv1Mode=($script:QpMode -eq 'AV1 Base Q Index');$isMpeg4Mode=($script:QpMode -eq 'MPEG-4 VOP Quantizer')
    if($null -ne $qpBox){$qpBox.Text=if($isAv1Mode){'AV1 Base Q Index distribution'}elseif($isMpeg4Mode){'MPEG-4 VOP quantizer distribution'}else{'DRF distribution (frame-level SliceQPY)'}}
...
function Render {
    ...
    if($script:ReadyMedia){$lines+=@('','[ LSD 2.0 Canonical Model ]','',('Status:                {0}' -f $script:ReadyModelStatus),('Adapter:               {0}' -f $script:ReadyMedia.AdapterName),('Canonical tracks:      {0} (video {1}, audio {2}, subtitle {3})' -f $script:ReadyMedia.Tracks.Count,$script:ReadyMedia.CountType('video'),$script:ReadyMedia.CountType('audio'),$script:ReadyMedia.CountType('subtitle')),'Routing:               Canonical media pipeline active','Sample model:          Active canonical video and audio sample indexes')}
    if($script:ReadyMedia -and $script:NativeInfo.Container -eq 'AVI') {
        $aviVideo=@($script:ReadyMedia.Tracks | Where-Object { $_.TrackType -eq 'video' } | Select-Object -First 1)
        $lines+=@('','[ Native AVI Sample Index ]','',('Video samples:         {0:N0}' -f $(if($aviVideo){$aviVideo.Samples.Count}else{0})),('Index mode:            {0}' -f $script:AviProbe.IndexMode),('OpenDML AVI:           {0}' -f $(if($script:AviProbe.OpenDml){'Yes'}else{'No'})),('RIFF / AVIX segments:  {0} / {1}' -f $script:AviProbe.RiffSegments,$script:AviProbe.AvixSegments),('Standard indexes:      {0}' -f $script:AviProbe.StandardIndexes),('Legacy idx1:           {0}' -f $(if($script:AviProbe.LegacyIndexPresent){'Present'}else{'Not present'})),'Video mapping:         XVID/DIVX/DX50/FMP4/MP4V => MPEG-4 Part 2 ASP; H264/X264/AVC1/DAVC => AVC Annex B')
    }
    if($script:ReadyMedia -and $script:NativeInfo.Container -eq 'MP4 / MOV') {
        ...
        $lines+=@('','[ Native MP4 Sample Index ]','',('Video samples:         {0:N0}' -f $mp4SampleCount),('Timecode scale:        {0:N0} ns/tick' -f $script:ReadyMedia.TimecodeScale),'Sample table mode:     stsc + stsz + stco/co64 + stts/ctts + stss',$(if($nativeVideo -and $nativeVideo.CodecId -eq 'V_AV1'){'AV1 sample entry:     av01 + av1C'}elseif(@($script:NativeInfo.Tracks|Where-Object{$_.CodecId -eq 'A_AC3'}).Count){'Audio sample entry:   ac-3 + dac3'}elseif(@($script:NativeInfo.Tracks|Where-Object{$_.CodecId -eq 'A_EAC3'}).Count){'Audio sample entry:   ec-3 + dec3'}elseif(@($script:NativeInfo.Tracks|Where-Object{$_.CodecId -eq 'A_MPEG/L3'}).Count){'Audio sample entry:   MP3 / MPEG Audio'}elseif(@($script:NativeInfo.Tracks|Where-Object{$_.CodecId -eq 'A_PCM'}).Count){'Audio sample entry:   PCM / LPCM'}else{'Codec configuration:  native MP4 sample entry'}),'Fragmented MP4:        Not supported in version 2.0')
    }
    ...
    if($script:NativeInfo){
        ...
        if($nativeVideo -and $nativeVideo.CodecPrivate){
            ...
            } elseif($nativeVideo.CodecId -eq 'V_MPEG4/ISO/AVC') {
    ...
    if($video){
        ...
        if($script:PacketResult){
            ...
            $lines+=@("Packet engine:       $script:PacketEngine","Packets:            $('{0:N0}' -f $r.Count)","Key packets:        $('{0:N0}' -f $r.Key)","Packet payload:     $('{0:N0}' -f $r.Total) bytes ($(Bytes $r.Total))","Codec configuration: $('{0:N0}' -f $codecPrivateBytes) bytes","Encoded video total: $('{0:N0}' -f $encodedVideoTotal) bytes ($(Bytes $encodedVideoTotal))","Bitrate:            $(Rate $r.Bitrate)",$(if($fv -gt 0){"Qf:                 $('{0:N6}' -f $qf)"}else{"Qf:                 N/A"}),"Packet size:        avg $(Bytes ([long]$r.Average)), min $(Bytes $r.Minimum), max $(Bytes $r.Maximum)",$(if($r.Key -gt 1){"Key interval:       avg $('{0:N2}' -f $r.GopAverage), min $('{0:N0}' -f $r.GopMinimum), max $('{0:N0}' -f $r.GopMaximum)"}else{"Key interval:       N/A (fewer than two key packets)"}))
        }
    }
    $nativeAudioTracks=@($script:NativeInfo.Tracks|Where-Object{$_.Kind -eq 'audio'});$audioOrdinal=0;foreach($audio in $audios){$lang=if($audio.tags.language){$audio.tags.language}else{'und'};$codec=if($audio.profile -and $audio.codec_name -eq 'aac'){$audio.profile}else{"$(NA $audio.codec_long_name)"};if($audio.profile -and $audio.codec_name -ne 'aac'){$codec+=' '+$audio.profile};$nativeAudio=if($audioOrdinal -lt $nativeAudioTracks.Count){$nativeAudioTracks[$audioOrdinal]}else{$null};if($nativeAudio -and $nativeAudio.CodecId -like 'A_PCM*'){$codec=if($nativeAudio.PcmFloat){'PCM floating-point'}else{'PCM signed integer'}};$audioOrdinal++;$outputRate=if($nativeAudio -and $nativeAudio.OutputSampleRate -gt 0){$nativeAudio.OutputSampleRate}elseif($audio.sample_rate){$audio.sample_rate}else{$null};$outputChannels=if($nativeAudio -and $nativeAudio.AudioChannels -gt 0){$nativeAudio.AudioChannels}elseif($audio.channels){$audio.channels}else{$null};$outputLayout=if($audio.channel_layout){$audio.channel_layout}elseif($outputChannels -eq 1){'mono'}elseif($outputChannels -eq 2){'stereo'}elseif($outputChannels){"$outputChannels channels"}else{$null};$audioLines=@('',("[ Audio #{0} ]" -f $audio.index),'',"Codec:              $codec",$(if($nativeAudio -and $nativeAudio.CoreSampleRate -gt 0){"Core sample rate:   $($nativeAudio.CoreSampleRate) Hz"}else{"Core sample rate:   N/A"}),$(if($outputRate){"Output sample rate: $outputRate Hz"}else{"Output sample rate: N/A"}),$(if($nativeAudio -and $nativeAudio.CoreAudioChannels -gt 0){"Core channels:      $($nativeAudio.CoreAudioChannels)"}else{"Core channels:      N/A"}),"Output channels:    $(NA $outputChannels)","Output layout:      $(NA $outputLayout)",$(if($nativeAudio -and $nativeAudio.CodecId -like 'A_PCM*'){"PCM format:         $(if($nativeAudio.PcmFloat){'floating-point'}else{'signed integer'}), $($nativeAudio.PcmBitDepth)-bit $(if($nativeAudio.PcmBigEndian){'big-endian'}else{'little-endian'})"}else{$null}),$(if($audio.codec_name -eq 'aac' -and $nativeAudio){"SBR / PS:           $(if($nativeAudio.Sbr){'Yes'}else{'No'}) / $(if($nativeAudio.Ps){'Yes'}else{'No'})"}elseif($audio.codec_name -eq 'aac'){"SBR / PS:           N/A"}else{$null}),"Bitrate:            $(Rate $audio.bit_rate)","Language:           $lang");$lines+=@($audioLines|Where-Object{$null -ne $_})}
...
$(if($hd.SliceParsed -eq $r.Count -and $hd.SliceRejected -eq 0 -and ($r.I+$r.P+$r.B) -eq $r.Count){'HEVC I/P/B validation: Yes'}else{'HEVC I/P/B validation: Incomplete'}))}else{$lines+=@('','[ Native Stream Analysis ]','',"Total frames:       $('{0:N0}' -f $r.Count)","I-frames:           $('{0:N0}' -f $r.I) ($(Pct $r.I $r.Count))","P-frames:           $('{0:N0}' -f $r.P) ($(Pct $r.P $r.Count))","B-frames:           $('{0:N0}' -f $r.B) ($(Pct $r.B $r.Count))","Other frames:       $('{0:N0}' -f $r.Other) ($(Pct $r.Other $r.Count))","Key frames:         $('{0:N0}' -f $r.Key)",$(if($r.Key -gt 1){"GOP length:         avg $('{0:N2}' -f $r.GopAverage), min $('{0:N0}' -f $r.GopMinimum), max $('{0:N0}' -f $r.GopMaximum)"}else{"GOP length:         N/A (fewer than two key frames)"}),"Packet/frame match: $match")}}
    ...
    if($nativeVideo -and ($nativeVideo.CodecId -eq 'V_MPEG4/ISO/AVC' -or $nativeVideo.CodecId -eq 'V_MPEGH/ISO/HEVC') -and $script:SliceQpDiagnostic -and $script:QpCounts.Count){
        $sd=$script:SliceQpDiagnostic
    ...
    if($nativeVideo.CodecId -eq 'V_MPEG4/ISO/ASP' -and $script:Mpeg4Diagnostic){$md=$script:Mpeg4Diagnostic;$lines+=@('','[ Native MPEG-4 Part 2 Analysis ]','',"VOP headers parsed:    $('{0:N0}' -f $md.Parsed)","VOP headers rejected:  $('{0:N0}' -f $md.Rejected)","Packed samples:        $('{0:N0}' -f $md.Packed)","N-VOP events:          $('{0:N0}' -f $md.NVop)","Average VOP quantizer: $('{0:N6}' -f $md.Average)","Standard deviation:    $('{0:N6}' -f $md.StdDev)","Minimum / maximum:     $($md.Minimum) / $($md.Maximum)","First VOP failure:     $(if($md.FirstError){$md.FirstError}else{'None'})","VOP validation:        $(if($md.Rejected -eq 0 -and $md.Parsed -gt 0){'PASS'}else{'INCOMPLETE'})")}
    if($script:AudioDiagnostic){$ad=$script:AudioDiagnostic;$lines+=@('','[ Native Canonical Audio Analysis ]','',"Codec family:          $($ad.CodecFamily)","Canonical samples:     $('{0:N0}' -f $ad.Samples)","Audio payload:         $('{0:N0}' -f $ad.PayloadBytes) bytes ($(Bytes $ad.PayloadBytes))","Sample size:           avg $(Bytes ([long]$ad.Average)), min $(Bytes $ad.Minimum), max $(Bytes $ad.Maximum)","Measured bitrate:      $(Rate $ad.Bitrate)","Rejected samples:      $('{0:N0}' -f $ad.Rejected)","First audio failure:   $(if($ad.FirstError){$ad.FirstError}else{'None'})","Sample accounting:     $(if($ad.Complete){'PASS'}else{'INCOMPLETE'})");if($ad.CodecFamily -eq 'MP3'){$ver=if($ad.MpegVersion -eq 1){'MPEG-1'}elseif($ad.MpegVersion -eq 2){'MPEG-2'}else{'MPEG-2.5'};$lines+=@('', '[ Native MPEG Audio Analysis ]','',"MPEG version / layer:  $ver / Layer $($ad.Layer)","Frames parsed:         $('{0:N0}' -f $ad.MpegAudioFrames)","Frame payload:         $('{0:N0}' -f $ad.MpegAudioBytes) bytes","Decoded duration:      $([TimeSpan]::FromSeconds([double]$ad.MpegAudioPcmSamples/$ad.SampleRate).ToString())","Frame bitrate range:   $($ad.MinimumFrameBitrate) .. $($ad.MaximumFrameBitrate) kbps","Sample rate:           $($ad.SampleRate) Hz","Channels:              $($ad.Channels)","Frame validation:      $(if($ad.Rejected -eq 0 -and $ad.MpegAudioFrames -gt 0){'PASS'}else{'INCOMPLETE'})")}}
    if($script:AudioDiagnostic -and $script:AudioDiagnostic.CodecFamily -eq 'PCM'){$pd=$script:AudioDiagnostic;$lines+=@('','[ Native PCM Audio Analysis ]','',"Sample format:          $($pd.PcmFormat)","Bit depth:              $($pd.PcmBitDepth) bit","Sample rate:            $($pd.PcmSampleRate) Hz","Channels:               $($pd.PcmChannels)","Block alignment:        $($pd.PcmBlockAlign) bytes","Decoded duration:       $([TimeSpan]::FromSeconds($pd.PcmDecodedDuration).ToString())","Expected bitrate:       $(Rate ($pd.PcmSampleRate*$pd.PcmBitDepth*$pd.PcmChannels))","Measured bitrate:       $(Rate $pd.Bitrate)","Rejected samples:       $('{0:N0}' -f $pd.Rejected)","Sample validation:      $(if($pd.Complete){'PASS'}else{'INCOMPLETE'})")}
    if($script:AudioDiagnostic -and ($script:AudioDiagnostic.CodecFamily -eq 'AC3' -or $script:AudioDiagnostic.CodecFamily -eq 'EAC3')){$dd=$script:AudioDiagnostic;$lines+=@('','[ Native Dolby Audio Analysis ]','',"Codec:                 $($dd.DolbyCodec)","Sample rate:           $($dd.DolbySampleRate) Hz","Channels:              $($dd.DolbyChannels)","Output layout:         $($dd.DolbyLayout)","Measured bitrate:      $(Rate $dd.Bitrate)","Decoded duration:      $([TimeSpan]::FromSeconds([double]$dd.DolbyPcmSamples/$dd.DolbySampleRate).ToString())","Syncframes parsed:     $('{0:N0}' -f $dd.DolbyFrames)","Syncframes rejected:   $('{0:N0}' -f $dd.DolbyRejected)","Frame validation:      $(if($dd.DolbyFrames -gt 0 -and $dd.DolbyRejected -eq 0 -and $dd.Rejected -eq 0){'PASS'}else{'INCOMPLETE'})")}
    $isAv1=($nativeVideo.CodecId -eq 'V_AV1');$isMpeg4=($nativeVideo.CodecId -eq 'V_MPEG4/ISO/ASP');$isHevc=($nativeVideo.CodecId -eq 'V_MPEGH/ISO/HEVC');$drfStatus=if($isAv1){if($script:Av1Diagnostic -and $script:Av1Diagnostic.QuantRejected -eq 0){'AV1 frame-level Base Q Index histogram generated successfully.'}else{'AV1 Base Q Index analysis incomplete.'}}elseif($isMpeg4 -and $script:QpCounts.Count){'MPEG-4 Part 2 VOP quantizer histogram generated successfully.'}elseif($script:QpCounts.Count){'Frame-level SliceQPY histogram generated successfully.'}elseif($isHevc){'HEVC SliceQPY was not generated.'}else{'Unavailable: complete native frame-level QP values were not obtained.'};$lines+=@('','[ Native Engine ]','',('Container demux:     '+$script:NativeInfo.Container),"Video configuration: $(if($isAv1){'AV1CodecConfigurationRecord + config OBUs'}elseif($isHevc){'HEVC hvcC / VPS / SPS / PPS record'}elseif($isMpeg4){'MPEG-4 Visual Object Layer / VOP headers'}else{'AVC SPS / PPS / VUI'})","Audio configuration: $(if($null -eq $nativeAudio){'None'}elseif($nativeAudio.CodecId -eq 'A_AAC'){'AAC AudioSpecificConfig'}elseif($nativeAudio.CodecId -eq 'A_MPEG/L3'){'MPEG Audio frame headers'}elseif($nativeAudio.CodecId -eq 'A_AC3' -or $nativeAudio.CodecId -eq 'A_EAC3'){'Dolby syncframe headers'}elseif($nativeAudio.CodecId -like 'A_PCM*'){'PCM format parameters'}elseif($script:NativeInfo.Container -eq 'AVI'){'AVI WAVEFORMATEX'}else{'Container audio metadata'})","Frame structure:     $(if($isAv1){'AV1 single-tile traversal + native frame-level Base Q Index'}elseif($isHevc){'HEVC NAL + first-slice I / P / B analysis'}elseif($isMpeg4){'MPEG-4 Part 2 I / P / B VOP analysis'}else{'Native H.264 I / P / B slices'})","Quantizer source:    $(if($isAv1){'Native frame-level AV1 Base Q Index'}elseif($isMpeg4){'Native MPEG-4 VOP quantizer'}else{'Native frame-level SliceQPY'})",'Processing model:    Single pass / in-memory safe','External tools:      None','','[ Quantizer Analysis ]','',$drfStatus)
    $labelWidth=29
...
function Build-NativeMetadata {
    ...
    foreach($t in $script:NativeInfo.Tracks){
        $codecName=switch($t.CodecId){'V_MPEG4/ISO/AVC'{'h264'};'V_MPEGH/ISO/HEVC'{'hevc'};'V_AV1'{'av1'};'V_MPEG4/ISO/ASP'{'mpeg4'};'A_MPEG/L3'{'mp3'};'A_AC3'{'ac3'};'A_EAC3'{'eac3'};'A_PCM'{'pcm'};'A_AAC'{'aac'};default{$t.CodecId}}
        $codecLong=switch($t.CodecId){'V_MPEG4/ISO/AVC'{'H.264 / AVC'};'V_MPEGH/ISO/HEVC'{'H.265 / HEVC'};'V_AV1'{'AV1'};'V_MPEG4/ISO/ASP'{'MPEG-4 Part 2 / XviD'};'A_MPEG/L3'{'MP3'};'A_AC3'{'Dolby Digital / AC-3'};'A_EAC3'{'Dolby Digital Plus / E-AC-3'};'A_PCM'{if($t.PcmFloat){'PCM floating-point'}else{'PCM signed integer'}};'A_AAC'{'AAC'};default{$t.CodecId}}
...
function Start-Meta($path) {
    ...
    try {
        $script:NativeInfo=[NativeMediaProbe]::Inspect($path)
        if($script:NativeInfo.Container -eq 'MP4 / MOV'){
            ...
            $status.Text=if($script:NativeInfo.Container -eq 'AVI'){'Reading AVI RIFF headers and idx1 index...'}else{'Reading MP4 sample tables and building index...'}
            Log 'MP4 preparation started on a background thread; deep analysis will continue automatically.'
            return
        } elseif($script:NativeInfo.Container -eq 'AVI'){
            $script:AviProbe=$null;$script:ReadyMedia=$null
            $script:Mp4PrepJob=New-Object NativeAviPreparationJob
            $script:Mp4PrepJob.Start($path)
            $bar.Style='Marquee';$bar.MarqueeAnimationSpeed=25
            $status.Text='Reading AVI RIFF headers and idx1 index...'
            Log 'AVI 1.0 preparation started on a background thread; canonical analysis will continue automatically.'
            return
        } elseif($script:NativeInfo.Container -eq 'Matroska / WebM'){
            $script:Mp4Probe=$null;$script:AviProbe=$null;$script:ReadyMedia=[LsdMatroskaAdapter]::Adapt($path,$script:NativeInfo)
        } else { throw 'The native-only build currently supports Matroska / WebM, AVI 1.0, and unfragmented MP4 / M4V / MOV files.' }
...
function Update-CanonicalAudioAnalysis {
    $script:AudioDiagnostic=$null
    if(-not $script:ReadyMedia -or -not $script:File){return}
    $audioTrack=@($script:ReadyMedia.Tracks | Where-Object { $_.TrackType -eq 'audio' })[0]
    if($null -eq $audioTrack -or $audioTrack.Samples.Count -eq 0){return}
    $nativeAudio=@($script:NativeInfo.Tracks | Where-Object { $_.Kind -eq 'audio' })[0]
    $r=[CanonicalAudioAnalyzer]::Analyze($script:File,$audioTrack,[double]$script:Duration)
    $script:AudioDiagnostic=$r
    if($nativeAudio -and $r.Bitrate -gt 0){$nativeAudio.AudioPayloadBytes=$r.PayloadBytes;$nativeAudio.AudioBitrate=$r.Bitrate;if($r.SampleRate -gt 0){$nativeAudio.CoreSampleRate=$r.SampleRate;$nativeAudio.OutputSampleRate=$r.SampleRate;$nativeAudio.SamplingRate=$r.SampleRate};if($r.Channels -gt 0){$nativeAudio.CoreAudioChannels=$r.Channels;$nativeAudio.AudioChannels=$r.Channels;$nativeAudio.Channels=$r.Channels};if($r.DolbySampleRate -gt 0){$nativeAudio.CoreSampleRate=$r.DolbySampleRate;$nativeAudio.OutputSampleRate=$r.DolbySampleRate;$nativeAudio.SamplingRate=$r.DolbySampleRate};if($r.DolbyChannels -gt 0){$nativeAudio.CoreAudioChannels=$r.DolbyChannels;$nativeAudio.AudioChannels=$r.DolbyChannels;$nativeAudio.Channels=$r.DolbyChannels}}
    Build-Base (Build-NativeMetadata)
}
        ...
        if($script:Mp4PrepJob) {
            ...
            if($script:Mp4PrepJob.Done) {
                ...
                try {
                    ...
                    elseif($null -eq $prep.Probe -or $null -eq $prep.Media){throw 'Container preparation returned no media model.'}
                    else {if($prep -is [NativeAviPreparationJob]){$script:AviProbe=$prep.Probe}else{$script:Mp4Probe=$prep.Probe};$script:NativeInfo=$prep.Probe.Info;$script:ReadyMedia=$prep.Media;Complete-NativeMetadata}
                }
                finally {$prep.Dispose()}
            }
        }
        elseif($script:ScanJob) {
            $rawProgress=[Math]::Min(99,[Math]::Max(0,$script:ScanJob.Progress))
            if($script:ScanPhase -eq 'container-index'){$bar.Value=[Math]::Min(49,[int]($rawProgress/2));$status.Text=('Phase 1/2 - indexing Matroska samples | {0}% | {1:N0} samples'-f$bar.Value,$script:ScanJob.Count)}
            elseif($script:NativeInfo.Container -eq 'Matroska / WebM'){$bar.Value=[Math]::Min(99,50+[int]($rawProgress/2));$status.Text=('Phase 2/2 - canonical codec analysis | {0}% | {1:N0} samples'-f$bar.Value,$script:ScanJob.Count)}
            else{$bar.Value=$rawProgress;$status.Text=('Canonical codec analysis | {0}% | {1:N0} samples'-f$bar.Value,$script:ScanJob.Count)}
            if($script:ScanJob.Done) {
                $j=$script:ScanJob;$script:ScanJob=$null
                if($script:ScanPhase -eq 'container-index'){
                    try {
                        if($script:Cancelled -or $j.ExitCode -ne 0){if(-not $script:Cancelled){throw ('Matroska sample indexing failed.'+[Environment]::NewLine+$j.Error)};Busy $false;$status.Text='Cancelled'}
                        else {
                            $nativeVideo=@($script:NativeInfo.Tracks | Where-Object { $_.Kind -eq 'video' })[0]
                            $nativeAudio=@($script:NativeInfo.Tracks | Where-Object { $_.Kind -eq 'audio' })[0]
                            if($null -eq $nativeVideo -or $null -eq $script:ReadyTrack -or $script:ReadyTrack.Samples.Count -eq 0){throw 'Matroska preparation returned no canonical video samples.'}
                            $script:ScanPhase='canonical'
                            Update-CanonicalAudioAnalysis
                            $script:ScanJob=New-Object NativeMatroskaPacketScan -ArgumentList ([double]$script:Duration),$nativeVideo,$script:ReadyTrack,([long]$script:ReadyMedia.TimecodeScale),$nativeAudio,$script:ReadyAudioTrack
                            $script:ScanJob.StartCanonical($script:File)
                            $bar.Value=50
                            $status.Text=('Phase 2/2 - analyzing {0:N0} canonical video samples...'-f $script:ReadyTrack.Samples.Count)
                        }
                    }
                    finally {$j.Dispose()}
                    return
                ...
                if(-not $script:Cancelled -and $j.ExitCode -eq 0) {
                    ...
                    $script:Mpeg4Diagnostic=[pscustomobject]@{Parsed=$c.Mpeg4VopParsed;Rejected=$c.Mpeg4VopRejected;Packed=$c.Mpeg4PackedSamples;NVop=$c.Mpeg4NVop;Average=$c.Mpeg4QuantAverage;StdDev=$c.Mpeg4QuantStdDev;Minimum=$c.Mpeg4QuantMinimum;Maximum=$c.Mpeg4QuantMaximum;FirstError=$c.Mpeg4FirstError;Histogram=$c.Mpeg4QuantHistogram}
                    $script:NativeHevcQpDiagnostic=$script:SliceQpDiagnostic
                    $script:PacketEngine=if($script:NativeInfo.Container -eq 'MP4 / MOV'){'Canonical LSD media pipeline (Native MP4 sample tables)'}elseif($script:NativeInfo.Container -eq 'AVI'){if($script:AviProbe -and $script:AviProbe.OpenDml){'Canonical LSD media pipeline (Native OpenDML AVI indexes)'}else{'Canonical LSD media pipeline (Native AVI idx1)'}}else{'Canonical LSD media pipeline'}
                    Finish-Scan $c.Result $c.Bins
                    Log ('LSD 2.0 pipeline: '+$script:NativeInfo.Container+' reader -> canonical sample index -> shared AVC/HEVC/AV1/MPEG-4 Part 2 analyzer.')
                }
                else { Log ('Native analysis failed. '+$j.Error) }
                $j.Dispose();$script:ScanPhase='idle';$elapsed=([DateTime]::UtcNow-$script:StageStarted).TotalSeconds;$speed=if($elapsed -gt 0){(Get-Item -LiteralPath $script:File).Length/1MB/$elapsed}else{0};$pps=if($elapsed -gt 0){$script:PacketResult.Count/$elapsed}else{0};Log ('Fast native analysis: {0:N2} s | {1:N1} MiB/s | {2:N0} packets/s.' -f $elapsed,$speed,$pps);Busy $false;$status.Text=('Complete - canonical pipeline | {0:N1} MiB/s | {1:N0} packets/s' -f $speed,$pps)
            }
        }
    }
    catch {
